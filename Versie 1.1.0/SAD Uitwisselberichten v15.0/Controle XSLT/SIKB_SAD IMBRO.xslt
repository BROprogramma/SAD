...
<xsl:stylesheet
    ...
    <xsl:template match="immetingen:Characteristic">        
        ...
        <xsl:copy-of select="sikb:checkLookupId(., $prGUID, 'indicator', 'Parameter', 'WARNING')"/> 
    </xsl:template>    
					...
					<xsl:for-each select="./sam:relatedSamplingFeature">
						...
						<xsl:choose>				
							<xsl:when test="$roleId = '9'">        
								...
								<!-- if mixedSample, loop it once, not multiple times. so check the first relationNr in the analysissample of the fieldsample -->	
    ...
    <xsl:template match="imsikb0101:Borehole">
		...
		<xsl:copy-of select="sikb:checkFilled(., $record, 'measurementObjectType', 'ERROR')"/>   	
		<xsl:choose>
		  ...
		  <xsl:otherwise>
			<xsl:copy-of select="sikb:checkExistence(., $record, 'name', 'ERROR')"/>
			<xsl:copy-of select="sikb:checkFilled(., $record, 'name', 'ERROR')"/>
			<xsl:copy-of select="sikb:checkExistence(., $record, 'geometry', 'ERROR')"/>        
			<xsl:copy-of select="sikb:checkFilled(., $record, 'geometry', 'ERROR')"/>
			...
			<xsl:copy-of select="sikb:checkExistence(., $record, 'startTime', 'ERROR')"/>      
			<xsl:copy-of select="sikb:checkFilled(., $record, 'startTime', 'ERROR')"/>
			<xsl:copy-of select="sikb:checkExistence(., $record, 'depth', 'ERROR')"/>
			
			<xsl:copy-of select="sikb:checkLength(., $record, 'name', 24, 'ERROR')"/>
			<xsl:copy-of select="sikb:checkLookupId(., $record, 'measurementObjectType', 'MeetObjectSoort', 'ERROR')"/>
    ...
    <xsl:template match="imsikb0101:Layer">
		...
		<!-- verzamel alle indicatoren die je zoekt -->
		<xsl:variable name="gevondenIndicators">                   
			<xsl:for-each select="$allCharacteristics">			         
				...
				<xsl:variable name="result" select="./om:result"/>
				
				<xsl:variable name="guids" select="concat('Layer: (', $prGUID, ') Characteristic: (',./@gml:id,')')"/>    
				
				<xsl:choose>
					<xsl:when test="$indicatorId = '3699' and contains(fn:lower-case($indicatorUrn),fn:lower-case('Parameter'))">        
						<!-- Valideren soil type/texture -->					                 
						<xsl:copy-of select="sikb:checkLookupId($result, $guids, 'classifiedResult', 'Bodemsoort', 'ERROR')"/>			
						<!-- add to gevondenIndicators -->
						<xsl:value-of select="'grondsoort'"/>
						<xsl:text> </xsl:text>					
					</xsl:when>
					...
					<xsl:when test="$indicatorId = '3688' and contains(fn:lower-case($indicatorUrn),fn:lower-case('Parameter'))">        
						...
						<xsl:if test="count($allCharacteristics[fn:lower-case(immetingen:indicator) = fn:lower-case('urn:immetingen:Parameter:id:3699')]) &lt; 1">
					...
					<xsl:when test="$indicatorId = '3690' and contains(fn:lower-case($indicatorUrn),fn:lower-case('Parameter'))">        
    ...
    <xsl:function name="sikb:checkFilled">
        ...
        <xsl:variable name="elementName" select="string($context/name())"/>
        <xsl:variable name="elementLocalName" select="string($context/local-name())"/>
        <xsl:variable name="message" select="string-join(('Het element', $field, 'bij', $elementLocalName, $prGUID, 'onbreekt'), ' ')"/>
...
<xsl:variable name="valueDate">
    ...
    <xsl:function name="sikb:checkLookupId" as="element()*">
        ...
        <xsl:variable name="checkLookupRecord">
            <xsl:choose>
                <xsl:when test="$lookupRecord and $lookupRecord/@status = 'Vervallen' and $lookupRecord/broSadImbro != 'true'">vervallen</xsl:when>
                <xsl:when test="$lookupRecord and $lookupRecord/broSadImbro != 'true'">niet geldig voor BRO SAD IMBRO kwaliteit</xsl:when>
